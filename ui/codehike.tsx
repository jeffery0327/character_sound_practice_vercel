...
type GridData = z.infer<typeof Schema>;
type GridCol = GridData['col'][number];

export function Grid(props: unknown) {
  const data = parseProps(props, Schema) as GridData;

  return (
    <div className="my-5 grid grid-cols-1 gap-6 lg:grid-cols-2 [&:first-child]:mt-0 [&:last-child]:mb-0">
      {data.col.map((col: GridCol, index: number) => (
...
export function Mdx({
  ...
  return (
    <Prose
      collapsed={collapsed}
      className="prose prose-sm prose-h1:font-medium prose-h2:font-medium prose-h3:font-medium prose-h4:font-medium prose-h5:font-medium prose-h6:font-medium prose-pre:mt-0 prose-pre:mb-0 prose-pre:rounded-none prose-pre:bg-transparent max-w-none"
    >
      <MdxComponent
